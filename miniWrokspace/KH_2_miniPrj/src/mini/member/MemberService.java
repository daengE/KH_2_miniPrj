...
public class MemberService {
	...
	public int join(MemberPetVo petVo) {
		...
		try {
			conn = JDBCTemplate.getConnection();
			result = new MemberDao().join(petVo, conn);
	...
	public int deleteMyPet(String petName) {
		// TODO
		// 반려동물 삭제 유효성 검사
		if(petName.length() < 1) {
		...
		try {
			conn = JDBCTemplate.getConnection();
			result = new MemberDao().deleteMyPet(petName, conn);
